...
## 范围
文章正文、封面、SEO 图片、系列封面；移除保存后单次登记及顺手删除素材。
...
## 实际运行验收
2026-09-11 Workspace Blog 独立组合启用当前源码，真实生命周期引用数 2→1→0→1→0、删除保护 409、解引用后删除 204 通过；仅清理本次测试文章和素材，原有素材保持。
CLI Playwright 桌面和移动端验证素材引用弹窗、正确站点绝对链接、无横向溢出和脚本异常；缩略图 naturalWidth=1 符合原始 1×1 GIF。修复本地命名 Backend 和 Account 图片代理后复验通过。
证据：E:/tmp/yueli-consumer-references-20260911/blog-lifecycle-report.json、blog-media-report.json 和 blog-media-{desktop,mobile}.png。
测试采用显式本地 Asset/Foundation 依赖覆盖；没有发布正式 SDK、生产部署或 Git 提交。
