...
type TaxonomyView struct {
	...
	PostCount   int    `json:"postCount"` // published posts under this taxonomy
}

// ── browse (public) ──────────────────────────────────────────────────────────

type ListTaxonomiesReq struct {
	g.Meta    `path:"/api/v1/taxonomies" method:"get" tags:"blog" summary:"List taxonomies (category/tag)"`
...
type CreateTaxonomyReq struct {
	g.Meta      `path:"/api/v1/taxonomies" method:"post" tags:"blog" summary:"Create a category/tag"`
	Name        string `json:"name" v:"required"`
	Taxonomy    string `json:"taxonomy" v:"required|in:category,tag"`
	Slug        string `json:"slug"` // optional; auto-derived from name when empty
...
type AssignTaxonomiesReq struct {
	g.Meta      `path:"/api/v1/posts/{id}/taxonomies" method:"put" tags:"blog" summary:"Set a post's taxonomies"`
...
type UpdateTaxonomyReq struct {
	g.Meta      `path:"/api/v1/taxonomies/{id}" method:"patch" tags:"blog" summary:"Update a taxonomy (admin)"`
	ID          string  `json:"id" in:"path" v:"required"`
	Name        *string `json:"name"`
	Slug        *string `json:"slug"`
	Description *string `json:"description"`
...
type DeleteTaxonomyReq struct {
	g.Meta `path:"/api/v1/taxonomies/{id}" method:"delete" tags:"blog" summary:"Delete a taxonomy (admin; refused if it has children)"`
	ID     string `json:"id" in:"path" v:"required"`
}
