...
func MustAPIMiddleware(limiter *ratelimit.Limiter) *goframeapi.Middleware {
	options := goframeapi.Options{
		...
		RateLimited: blogerr.DescriptorRateLimited,
		Validation:  blogerr.DescriptorValidation,
		Internal:    blogerr.DescriptorInternal,
	}
	if limiter != nil {
...
func MustRateLimiterFromEnvironment() *ratelimit.Limiter {
	...
	if raw := os.Getenv("BLOG_RATE_LIMIT_PER_MINUTE"); raw != "" {
		parsed, err := strconv.Atoi(raw)
		if err != nil || parsed < 0 {
			panic(fmt.Errorf("BLOG_RATE_LIMIT_PER_MINUTE must be a non-negative integer"))
		}
		limit = parsed
	}
