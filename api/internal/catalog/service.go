...
func (s *Service) ListManage(ctx context.Context, authorID, status, q string, taxonomyIDs []string, pinned, featured bool, page, size int) ([]*model.Post, int, int, int, error) {
	page, size = norm(page, size)
	items, total, err := s.dao.ListManage(ctx, authorID, status, q, taxonomyIDs, pinned, featured, size, (page-1)*size)
	return items, total, page, size, err
}
