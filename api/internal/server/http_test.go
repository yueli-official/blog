...
func TestBlogHTTPRoundTrip(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		rc.Close()
		id := jc.Get("data.post.id").String()
		t.AssertNE(id, "")
		t.Assert(jc.Get("data.post.status").String(), "draft")
		t.Assert(jc.Get("data.post.slug").String(), "hello-world")
		...
		r4.Close()
		t.Assert(j4.Get("data.post.title").String(), "Hello World")
		t.AssertNE(j4.Get("data.post.publishedAt").String(), "")
		t.Assert(j4.Get("data.post.excerpt").String(), "body") // auto-derived (no author-set excerpt)
		...
		ctok := gjson.New(rci.ReadAllString()).Get("data.uploadToken").String()
		rci.Close()
		t.AssertNE(ctok, "")
		rcf, err := op().Post(ctx, "/api/v1/posts/"+id+"/cover/finalize", g.Map{"uploadToken": ctok})
		t.AssertNil(err)
		t.Assert(rcf.StatusCode, 200)
		coverURL := gjson.New(rcf.ReadAllString()).Get("data.coverUrl").String()
		rcf.Close()
		t.AssertNE(coverURL, "")
		rcd, err := anon().Get(ctx, "/api/v1/posts/hello-world")
		t.AssertNil(err)
		t.AssertNE(gjson.New(rcd.ReadAllString()).Get("data.post.coverUrl").String(), "")
		rcd.Close()
		...
		itok := gjson.New(rii.ReadAllString()).Get("data.uploadToken").String()
		rii.Close()
		t.AssertNE(itok, "")
		rif, err := op().Post(ctx, "/api/v1/images/finalize", g.Map{"uploadToken": itok})
		t.AssertNil(err)
		t.Assert(rif.StatusCode, 200)
		t.AssertNE(gjson.New(rif.ReadAllString()).Get("data.url").String(), "")
		rif.Close()
		...
		t.AssertNil(err)
		t.Assert(gjson.New(rvd.ReadAllString()).Get("data.post.viewCount").Int(), 2)
		rvd.Close()
		...
		rrl.Close()
		revs := jrl.Get("data.items").Array()
		t.Assert(len(revs), 2)
		oldestRev := jrl.Get("data.items.1.id").String() // newest-first → index 1 is the "body" snapshot
		t.AssertNE(oldestRev, "")
		rrs, err := op().Post(ctx, "/api/v1/posts/"+id+"/revisions/"+oldestRev+"/restore", g.Map{})
		t.AssertNil(err)
		t.Assert(rrs.StatusCode, 200)
		t.Assert(gjson.New(rrs.ReadAllString()).Get("data.post.content").String(), "body")
		rrs.Close()
		...
		t.AssertNil(err)
		emptyID := gjson.New(re.ReadAllString()).Get("data.post.id").String()
		re.Close()
		...
		rtc.Close()
		taxID := jtc.Get("data.taxonomy.id").String()
		t.AssertNE(taxID, "")
		t.Assert(jtc.Get("data.taxonomy.slug").String(), "tech")
		...
		t.AssertNil(err)
		t.Assert(gjson.New(rtf.ReadAllString()).Get("data.total").Int(), 1)
		rtf.Close()

		// empty filter for an unknown taxonomy → zero
		rtz, err := anon().Get(ctx, "/api/v1/posts", g.Map{"taxonomy": "nope"})
		t.AssertNil(err)
		t.Assert(gjson.New(rtz.ReadAllString()).Get("data.total").Int(), 0)
		rtz.Close()
		...
		t.AssertNil(err)
		t.Assert(len(gjson.New(rtl.ReadAllString()).Get("data.items").Array()) >= 1, true)
		rtl.Close()
		...
		t.AssertNil(err)
		relID := gjson.New(rr2.ReadAllString()).Get("data.post.id").String()
		rr2.Close()
		t.AssertNE(relID, "")
		...
		rrel.Close()
		t.Assert(len(jrel.Get("data.items").Array()), 1)
		t.Assert(jrel.Get("data.items.0.slug").String(), "related-two") // self excluded
		...
		t.AssertNil(err)
		lonelyID := gjson.New(rln.ReadAllString()).Get("data.post.id").String()
		rln.Close()
		...
		t.AssertNil(err)
		t.Assert(len(gjson.New(rlnr.ReadAllString()).Get("data.items").Array()), 0)
		rlnr.Close()
		...
		t.AssertNil(err)
		t.Assert(gjson.New(rsd.ReadAllString()).Get("data.seo.metaTitle").String(), "MT")
		rsd.Close()

		// 8d. like toggle: like → likeCount 1 + liked true; unlike → 0 + false
		rlk, err := op().Post(ctx, "/api/v1/posts/hello-world/like", g.Map{})
		t.AssertNil(err)
		t.Assert(gjson.New(rlk.ReadAllString()).Get("data.liked").Bool(), true)
		rlk.Close()
		rld, err := op().Get(ctx, "/api/v1/posts/hello-world")
		t.AssertNil(err)
		jld := gjson.New(rld.ReadAllString())
		rld.Close()
		t.Assert(jld.Get("data.liked").Bool(), true)
		t.Assert(jld.Get("data.post.viewCount").Int(), 2) // unchanged by likes
		rlk2, err := op().Post(ctx, "/api/v1/posts/hello-world/like", g.Map{})
		t.AssertNil(err)
		t.Assert(gjson.New(rlk2.ReadAllString()).Get("data.liked").Bool(), false)
		rlk2.Close()

		// 8e. bookmark toggle
		rbk, err := op().Post(ctx, "/api/v1/posts/hello-world/bookmark", g.Map{})
		t.AssertNil(err)
		t.Assert(gjson.New(rbk.ReadAllString()).Get("data.bookmarked").Bool(), true)
		rbk.Close()
		rbd, err := op().Get(ctx, "/api/v1/posts/hello-world")
		t.AssertNil(err)
		t.Assert(gjson.New(rbd.ReadAllString()).Get("data.bookmarked").Bool(), true)
		rbd.Close()
		...
		t.Assert(gjson.New(ra1.ReadAllString()).Get("data.pending").Bool(), true)
		ra1.Close()
		rcl0, err := anon().Get(ctx, "/api/v1/posts/hello-world/comments")
		t.AssertNil(err)
		t.Assert(gjson.New(rcl0.ReadAllString()).Get("data.total").Int(), 0) // anon is pending
		rcl0.Close()
		rco, err := op().Post(ctx, "/api/v1/posts/hello-world/comments", g.Map{"content": "author hi"})
		t.AssertNil(err)
		jco := gjson.New(rco.ReadAllString())
		rco.Close()
		t.Assert(jco.Get("data.pending").Bool(), false)
		topID := jco.Get("data.comment.id").String()
		t.AssertNE(topID, "")
		...
		t.AssertNil(err)
		t.Assert(gjson.New(rcr.ReadAllString()).Get("data.pending").Bool(), false)
		rcr.Close()
		rcl1, err := anon().Get(ctx, "/api/v1/posts/hello-world/comments")
		t.AssertNil(err)
		jcl1 := gjson.New(rcl1.ReadAllString())
		rcl1.Close()
		t.Assert(jcl1.Get("data.total").Int(), 1)
		t.Assert(jcl1.Get("data.items.0.content").String(), "author hi")
		t.Assert(jcl1.Get("data.items.0.isMember").Bool(), true)
		t.Assert(len(jcl1.Get("data.items.0.replies").Array()), 1)
		t.Assert(jcl1.Get("data.items.0.replies.0.content").String(), "a reply")
		...
		rmp.Close()
		t.Assert(jmp.Get("data.total").Int(), 1)
		anonID := jmp.Get("data.items.0.id").String()
		t.AssertNE(anonID, "")
		...
		t.AssertNil(err)
		t.Assert(gjson.New(rcl2.ReadAllString()).Get("data.total").Int(), 2)
		rcl2.Close()
		...
		t.AssertNil(err)
		t.Assert(gjson.New(rcl3.ReadAllString()).Get("data.total").Int(), 1) // only anon top remains
		rcl3.Close()
		...
		t.Assert(gjson.New(rsub.ReadAllString()).Get("data.pending").Bool(), true)
		rsub.Close()
		...
		t.Assert(gjson.New(rcfm.ReadAllString()).Get("data.email").String(), subEmail)
		rcfm.Close()
		...
		t.AssertNil(err)
		digestID := gjson.New(rdp.ReadAllString()).Get("data.post.id").String()
		rdp.Close()
...
func TestAdminModeratesAnyComment(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		t.AssertNil(err)
		postID := gjson.New(rc.ReadAllString()).Get("data.post.id").String()
		rc.Close()
		t.AssertNE(postID, "")
		...
		jco := gjson.New(rco.ReadAllString())
		rco.Close()
		commentID := jco.Get("data.comment.id").String()
...
func TestBlogCommentGuard(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		t.AssertNil(err)
		id := gjson.New(rc.ReadAllString()).Get("data.post.id").String()
		rc.Close()
		rp, err := op().Patch(ctx, "/api/v1/posts/"+id, g.Map{"status": "published"})
		t.AssertNil(err)
		rp.Close()
		...
		t.Assert(gjson.New(rl.ReadAllString()).Get("data.pending").Bool(), true)
		rl.Close()
