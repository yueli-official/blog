...
VALUES
  ...
  ($2, 'contributor', 'active')`, testSub, testSub2)
		...
		t.Assert(j4.Get("canEdit").Bool(), false)
		...
		t.Assert(strings.Contains(coverURL, "format=webp&preset=home&v=1"), true)
		...
		t.Assert(strings.Contains(inlineURL, "format=webp&preset=inline&v=1"), true)
		rif.Close()
		...
		t.Assert(jld.Get("canEdit").Bool(), true)
		...
		t.Assert(jcl2.Get("items.0.isAnonymous").Bool(), true)
		rcl2.Close()
		// Public threads support deterministic server-side oldest/newest ordering.
		roldest, err := anon().Get(ctx, "/api/v1/posts/hello-world/comments", g.Map{"sortOrder": "asc"})
		t.AssertNil(err)
		t.Assert(gjson.New(roldest.ReadAllString()).Get("items.0.id").String(), anonID)
		roldest.Close()
		rnewest, err := anon().Get(ctx, "/api/v1/posts/hello-world/comments", g.Map{"sortOrder": "desc"})
		t.AssertNil(err)
		t.Assert(gjson.New(rnewest.ReadAllString()).Get("items.0.id").String(), topID)
		rnewest.Close()
