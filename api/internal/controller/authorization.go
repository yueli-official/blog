...
func (*Authorization) GetAuthorizationConsole(
	...
	revisions, err := service.Runtime().ListPolicyRevisions(ctx, authorization.PolicyRevisionListQuery{
		Actor: service.Subject(ctx), ScopeID: blogauthz.RootScopeID, Limit: 500,
	})
	if err != nil {
		return nil, mapAuthorizationError(err)
	}
	...
	snapshot, err := service.Runtime().GetPolicySnapshot(ctx, authorization.PolicySnapshotQuery{
		Actor: service.Subject(ctx), Revision: selected.Number, ScopeID: blogauthz.RootScopeID,
	})
	if err != nil {
		return nil, mapAuthorizationError(err)
	}
	applications, err := service.Runtime().ListApplications(ctx, authorization.ApplicationListQuery{
		Actor: service.Subject(ctx), ScopeID: blogauthz.RootScopeID,
	...
	return &v1.GetAuthorizationConsoleRes{
		...
		Capabilities:   blogCapabilityViews(),
	}, nil
}
...
func (*Authorization) CreateAuthorizationRole(
	...
	role, err := service.Runtime().CreateRole(ctx, authorization.CreateRoleCommand{
		...
		ScopeID: blogauthz.RootScopeID, Capabilities: capabilityKeys(req.Capabilities),
...
func (*Authorization) ListRequestableRoles(
	...
	roles, err := service.Runtime().ListRequestableRoles(ctx, authorization.RequestableRoleQuery{
		Subject: service.Subject(ctx), ScopeID: blogauthz.RootScopeID,
	})
	if err != nil {
		return nil, mapAuthorizationError(err)
	}
...
func (*Authorization) ApplyForRole(
	...
	application, err := service.Runtime().Apply(ctx, authorization.ApplyCommand{
		...
		ScopeID: blogauthz.RootScopeID, Reason: req.Reason,
	})
	if err != nil {
		return nil, mapAuthorizationError(err)
	}
...
func (*Authorization) ListMyApplications(
	...
	page, err := service.Runtime().ListApplications(ctx, authorization.ApplicationListQuery{
		Actor: subject, Subject: subject, ScopeID: blogauthz.RootScopeID, Limit: 100,
	})
	if err != nil {
		return nil, mapAuthorizationError(err)
	}
...
func (*Authorization) GrantAuthorizationRole(
	...
	grant, err := service.Runtime().Grant(ctx, authorization.GrantCommand{
		...
		Role:   authorization.RoleKey(req.Role), ScopeID: blogauthz.RootScopeID,
...
func blogCapabilityViews() []v1.AuthorizationCapabilityView {
	return []v1.AuthorizationCapabilityView{
		{Key: string(blogauthz.CapabilityPostCreate), DisplayName: "创建文章"},
		{Key: string(blogauthz.CapabilityPostRead), DisplayName: "查看可管理文章"},
		{Key: string(blogauthz.CapabilityPostUpdate), DisplayName: "编辑自己的文章"},
		{Key: string(blogauthz.CapabilityPostPublish), DisplayName: "发布自己的文章"},
		{Key: string(blogauthz.CapabilityPostArchive), DisplayName: "归档自己的文章"},
		{Key: string(blogauthz.CapabilityPostDelete), DisplayName: "删除自己的文章"},
		{Key: string(blogauthz.CapabilityPostFlagsManage), DisplayName: "管理推荐与置顶"},
		{Key: string(blogauthz.CapabilitySeriesCreate), DisplayName: "创建系列"},
		{Key: string(blogauthz.CapabilitySeriesUpdate), DisplayName: "编辑自己的系列"},
		{Key: string(blogauthz.CapabilitySeriesDelete), DisplayName: "删除自己的系列"},
		{Key: string(blogauthz.CapabilityCommentModerate), DisplayName: "审核文章评论"},
		{Key: string(blogauthz.CapabilityCommentDelete), DisplayName: "删除文章评论"},
		{Key: string(blogauthz.CapabilityTagCreate), DisplayName: "创建标签"},
		{Key: string(blogauthz.CapabilityTaxonomyManage), DisplayName: "治理分类与标签"},
		{Key: string(blogauthz.CapabilitySiteSettingsManage), DisplayName: "管理站点设置"},
		{Key: string(blogauthz.CapabilityAssetSettingsManage), DisplayName: "管理资源配置"},
	}
}
