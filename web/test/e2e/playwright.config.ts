...
const blogPort = process.env.LOCAL_BLOG_WEB_PORT?.trim() || "3002";
const blogURL =
  process.env.BLOG_E2E_URL?.trim() ||
  `http://${localHost}:${blogPort}`;
process.env.BLOG_E2E_SITES ||= JSON.stringify([
  {
    slug: "blog-main",
    product: "blog",
    url: blogURL,
...
if (!supportedSuites.has(suite))
  throw new Error(`不支持的 Blog E2E suite：${suite}`);

const runID =
  process.env.BLOG_E2E_RUN_ID?.trim() ||
