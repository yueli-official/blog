...
async function load() {
  loading.value = true
  try {
    const r = await call<CommentList>(`/api/v1/posts/${props.slug}/comments`, { query: { page: 1, size: 100 } })
    items.value = r.items
    total.value = r.total
  } catch {
...
<template>
  <section class="mt-16 border-t border-default pt-10">
    <h2 class="font-display mb-6 flex items-center gap-2 text-xl font-semibold text-highlighted">
      <UIcon name="i-tabler-messages" class="size-5 text-primary" />
      评论
      <span v-if="total" class="text-base font-normal text-muted">{{ total }}</span>
    </h2>

    <div v-if="commentStatus === 0" class="rounded-xl border border-dashed border-default py-8 text-center text-sm text-muted">
    ...
    <template v-else>
      ...
      <div v-else-if="!items.length" class="rounded-xl border border-dashed border-default py-10 text-center">
      ...
      <ul v-else class="space-y-8">
        <li v-for="c in items" :key="c.id">
          <div class="flex gap-3">
            <UAvatar :text="authorInitial(c.authorName)" size="sm" class="shrink-0" />
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2 text-sm">
                <span class="font-medium text-highlighted">{{ c.authorName }}</span>
                <UBadge v-if="c.isMember" label="会员" color="primary" variant="subtle" size="sm" />
              ...
              <UButton
                :icon="replyTo === c.id ? 'i-tabler-x' : 'i-tabler-corner-down-right'"
              ...
              <CommentForm v-if="replyTo === c.id" :slug="slug" :parent-id="c.id" compact class="mt-3" @submitted="onSubmitted" />

              <ul v-if="c.replies?.length" class="mt-4 space-y-4 border-l-2 border-default pl-4">
                <li v-for="r in c.replies" :key="r.id" class="flex gap-2.5">
                  <UAvatar :text="authorInitial(r.authorName)" size="2xs" class="mt-0.5 shrink-0" />
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2 text-sm">
                      <span class="font-medium text-highlighted">{{ r.authorName }}</span>
                      <UBadge v-if="r.isMember" label="会员" color="primary" variant="subtle" size="sm" />
                      <span class="text-dimmed">·</span>
                      <ClientOnly><span class="text-muted">{{ rel(r.createdAt) }}</span><template #fallback><span /></template></ClientOnly>
                    </div>
                    <p class="mt-1 whitespace-pre-wrap text-sm leading-relaxed text-default">{{ r.content }}</p>
                  </div>
                </li>
              </ul>
            </div>
