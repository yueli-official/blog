...
const { data: cats } = await useAsyncData("home-categories", () =>
  call<ListTaxonomies>("/api/v1/taxonomies", {
    query: { taxonomy: "category" },
  }),
);
const { data: tagData } = await useAsyncData("home-tags", () =>
  call<ListTaxonomies>("/api/v1/taxonomies", { query: { taxonomy: "tag" } }),
);
const { data: homeConfigData } = await useAsyncData("home-config", () =>
...
const { data: featuredData } = await useAsyncData("home-featured", () =>
  call<ListPosts>("/api/v1/posts", { query: { featured: true, size: 8 } }),
);
const { data: seriesData } = await useAsyncData("home-series", () =>
...
const { data, pending } = await useAsyncData(
  ...
  () =>
    call<ListPosts>("/api/v1/posts", {
      query: { taxonomy: taxonomy.value || undefined, page: page.value, size },
    }),
  { watch: [taxonomy, page] },
);
// discovery widgets (sidebar): 热门 by views, 随机 refreshable.
const { data: popularData } = await useAsyncData("home-popular", () =>
  call<ListPosts>("/api/v1/posts", { query: { sort: "popular", size: 10 } }),
);
const {
  data: randomData,
  refresh: refreshRandom,
  pending: randomPending,
} = await useAsyncData("home-random", () =>
  call<ListPosts>("/api/v1/posts", { query: { sort: "random", size: 5 } }),
);
...
const series = computed<SeriesView[]>(() =>
  [...(seriesData.value?.items ?? [])]
    .sort((a, b) => b.postCount - a.postCount)
    .slice(0, HOME_SERIES_LIMIT),
);
const tags = computed(() =>
  [...(tagData.value?.items ?? [])]
    .sort((a, b) => b.postCount - a.postCount)
...
const lead = computed<PostView | undefined>(
  () => featured.value[activeFeatured.value] ?? featured.value[0],
);
...
const homeCats = computed(() =>
  [...(cats.value?.items ?? [])]
    .filter((c) => !c.parentId)
    .sort((a, b) => b.postCount - a.postCount)
    .slice(0, HOME_CAT_LIMIT),
);
...
function startFeat() {
  ...
  featTimer = setInterval(() => {
    activeFeatured.value = (activeFeatured.value + 1) % featured.value.length;
  }, 5000);
}
function pickFeat(i: number) {
  activeFeatured.value = i;
  startFeat();
}
onMounted(startFeat);
...
<template>
  <div class="space-y-14 sm:space-y-20" data-blog-home-ready>
    <h1 v-if="!lead" class="sr-only">{{ homeConfig.siteTitle }}</h1>
    ...
    >
      ...
      <div
        v-if="lead"
        class="flex flex-col gap-4 md:h-full md:min-h-0"
        @mouseenter="stopFeat"
        @mouseleave="startFeat"
      >
        <NuxtLink
          :to="`/posts/${lead.slug}`"
        ...
        >
          ...
          >
            <div :key="lead.id" class="flex flex-1 flex-col md:min-h-0">
              <div
                class="relative aspect-[16/9] overflow-hidden md:aspect-auto md:min-h-0 md:flex-1"
              >
                <img
                  v-if="lead.coverUrl"
                ...
                <div
                  v-else
                  class="blog-cover-placeholder relative size-full bg-gradient-to-br from-primary/15 via-primary/5 to-elevated"
                >
                  <UIcon
                    name="i-tabler-feather"
                    class="blog-cover-icon absolute -bottom-8 -right-6 size-48 text-primary/10"
                  />
                </div>
                <span
                  class="absolute left-4 top-4 inline-flex items-center gap-1 rounded-full bg-default/85 px-3 py-1 text-xs font-semibold text-primary shadow-sm backdrop-blur"
                >
              ...
              <div class="flex flex-col p-5 sm:p-6 md:flex-none">
                <h1
                  class="font-display line-clamp-1 text-xl font-bold leading-tight text-highlighted transition group-hover:text-primary sm:text-2xl"
                >
                  {{ lead.title }}
                </h1>
                <p class="mt-2 line-clamp-2 min-h-[2lh] text-sm text-muted">
                  {{ lead.excerpt }}
                </p>
                <div
          ...
          >
            <span
              ...
              :class="
                i === activeFeatured
                  ? 'w-6 bg-primary'
                  : 'w-2 bg-primary/40 group-hover:bg-primary/60'
              "
            />
          </button>
        </div>
      ...
      <div class="flex flex-col md:h-full md:min-h-0">
        ...
        <ul class="flex flex-1 flex-col divide-y divide-default">
          <li v-for="(p, i) in secondary" :key="p.id" class="md:flex-1">
            <NuxtLink
              :to="`/posts/${p.slug}`"
              class="group flex items-center gap-4 py-3 md:h-full"
            >
              <span
                class="font-display w-7 shrink-0 text-lg font-bold tabular-nums text-primary"
    ...
    <section
      class="space-y-12 lg:grid lg:gap-10 lg:space-y-0 lg:grid-cols-[minmax(0,1fr)_280px]"
    >
      ...
      <div class="min-w-0 space-y-14 sm:space-y-16">
        ...
        <div v-if="series.length">
          ...
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <article
              v-for="s in series"
              :key="s.id"
              class="flex min-w-0 gap-4 rounded-lg border border-default p-4 transition hover:border-primary/40 hover:shadow-md"
            >
              <NuxtLink
                :to="`/series/${s.slug}`"
                :aria-label="`查看专题：${s.name}`"
                class="relative aspect-square w-24 shrink-0 overflow-hidden rounded-md sm:w-28"
              >
                <img
                  v-if="s.coverUrl"
                  :src="s.coverUrl"
                ...
                <div
                  v-else
                  class="blog-cover-placeholder grid size-full place-items-center bg-gradient-to-br from-primary/20 to-primary/5"
                >
                  <UIcon
                    name="i-tabler-stack-2"
                    class="blog-cover-icon size-9 text-primary/50"
                  />
                </div>
              </NuxtLink>
              <div class="flex min-w-0 flex-1 flex-col">
                <NuxtLink
                  :to="`/series/${s.slug}`"
                  class="font-display line-clamp-1 font-semibold text-highlighted transition hover:text-primary"
                  >{{ s.name }}</NuxtLink
                >
                ...
                <ul class="mt-2 space-y-1.5">
                  <li
                    v-for="rp in (s.recentPosts || []).slice(0, 2)"
                    :key="rp.id"
                  >
                    <NuxtLink
                      :to="`/posts/${rp.slug}`"
                      class="flex items-center gap-1.5 text-sm text-muted transition hover:text-primary"
                    >
                      <UIcon
                        name="i-tabler-point-filled"
                        class="size-3 shrink-0 text-primary/40"
                      /><span class="min-w-0 line-clamp-1">{{ rp.title }}</span>
                    </NuxtLink>
                  </li>
                  <li
                    v-if="!(s.recentPosts || []).length"
        ...
        <div>
          ...
          <div
            ...
            class="rounded-lg border border-dashed border-default py-24 text-center"
          >
            <div
              class="mx-auto grid size-14 place-items-center rounded-lg bg-primary/10 text-primary"
            >
              <UIcon name="i-tabler-feather" class="size-7" />
            </div>
            <p class="mt-4 font-medium text-highlighted">这里还没有更多文章</p>
          </div>
          <template v-else>
            <div
              class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3"
            >
              <NuxtLink
                v-for="p in posts"
                :key="p.id"
                :to="`/posts/${p.slug}`"
                class="blog-article-card group flex min-w-0 gap-4 overflow-hidden rounded-lg border border-default transition hover:border-primary/40 hover:shadow-md sm:flex-col sm:gap-0"
              >
                <!-- cover: small thumb (row) on mobile, full 16:9 (column) on sm+ -->
                <div
                  class="relative w-28 shrink-0 self-stretch overflow-hidden bg-elevated sm:aspect-[16/9] sm:w-auto sm:self-auto"
                >
                  <img
                    v-if="p.coverUrl"
                    :src="coverThumbUrl(p)"
                  ...
                  >
                    <UIcon
                      name="i-tabler-feather"
                      class="blog-cover-icon size-10 text-primary/25"
                    />
                  </div>
                  <span
                    v-if="p.pinned"
                    class="absolute left-2 top-2 flex items-center gap-1 rounded-full bg-default/90 px-2 py-0.5 text-[11px] font-semibold text-primary backdrop-blur sm:left-3 sm:top-3 sm:px-2.5 sm:text-xs"
                    ><UIcon name="i-tabler-pin" class="size-3" />置顶</span
                  >
                </div>
                <div class="flex min-w-0 flex-1 flex-col py-3 pr-4 sm:p-5">
                  <h3
                    class="font-display line-clamp-2 text-base font-semibold leading-snug text-highlighted transition group-hover:text-primary sm:text-lg"
                  >
                    {{ p.title }}
                  </h3>
                  <p
                    v-if="p.excerpt"
                    class="mt-2 line-clamp-2 flex-1 text-sm text-muted max-sm:hidden"
                  >
                    {{ p.excerpt }}
                  </p>
                  <div
                    class="mt-2 flex items-center gap-3 text-xs text-muted sm:mt-4"
                  >
                    <ClientOnly
                      ><span>{{ rel(p.publishedAt || p.createdAt) }}</span
                      ><template #fallback><span /></template
      ...
      <aside class="space-y-8 lg:sticky lg:top-20 lg:self-start">
        <PostMiniList
          title="热门"
          icon="i-tabler-flame"
          :posts="popular.slice(0, 5)"
        />
        <PostMiniList
          title="随机阅读"
          icon="i-tabler-arrows-shuffle"
          :posts="random"
          :on-refresh="refreshRandom"
