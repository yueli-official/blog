...
import { SettingSection, SettingsLayout, SettingsSaveDock, type SettingsSaveDockMessages } from '@yueli/ui/settings/pattern'
...
onMounted(() => {
  mounted.value = true
  unbindBeforeUnload = bindSettingsBeforeUnload({ isDirty: () => settingsState.dirty.value })
})
onScopeDispose(() => unbindBeforeUnload?.())
useSettingsLeaveGuard({
  isDirty: () => settingsState.dirty.value,
...
<template>
  ...
  >
    ...
    <SettingSection v-if="showLoading" title="正在加载设置" description="读取当前站点的已保存配置。">
    ...
    <SettingSection v-else-if="section === 'home'" title="首页首屏" description="控制公开首页进入内容列表前的主文案。">
    ...
    <SettingSection v-else-if="section === 'footer' && !loadError" title="页脚内容" description="保持简短；用于全站底部的品牌说明与版权信息。">
      <div class="grid gap-4">
    ...
    <SettingSection v-else-if="!loadError" title="站点基础" description="这些字段用于导航品牌、站点说明和联系入口。">
