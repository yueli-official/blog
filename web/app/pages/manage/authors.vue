...
const showSkeleton = useMinimumLoading(
  computed(() => !mounted.value || mePending.value || loading.value),
);
const authors = computed(() => data.value?.authors ?? []);
const active = computed(() =>
  authors.value.filter((a) => a.status !== "pending"),
);
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(active.value.length / SIZE)),
);
const activePaged = computed(() =>
  active.value.slice((page.value - 1) * SIZE, page.value * SIZE),
);
...
const approve = (a: AdminAuthorView) =>
  act(a, () =>
    call(`/api/v1/admin/authors/${a.id}/approve`, { method: "POST", body: {} }),
  );
const remove = (a: AdminAuthorView) =>
  act(a, () => call(`/api/v1/admin/authors/${a.id}`, { method: "DELETE" }));
...
<template>
  <div>
    <PageHeader title="作者">
      <template #subtitle
        >管理本站作者。任何人发文需先「申请成为作者」,在此审批通过即成为作者。</template
      >
    </PageHeader>
    ...
    <template v-else>
      ...
      <template v-if="tab === 'active'">
        ...
        <div
          v-else
          class="blog-manage-panel divide-y divide-default overflow-hidden rounded-2xl"
        >
          <div
            v-for="a in activePaged"
            :key="a.id"
            class="blog-manage-row group flex items-center gap-3 px-4 py-3"
          >
            <UAvatar
              :text="nameOf(a).charAt(0).toUpperCase()"
              size="sm"
              class="shrink-0"
            />
        ...
        <CollectionDock v-if="active.length" label="作者分页">
          <template #selection
            ><span class="text-xs"
              >共 {{ active.length }} 位作者</span
            ></template
          >
          <template #pagination
      ...
      <template v-else>
        ...
        >
          <div
            v-for="a in pending"
            :key="a.id"
            class="blog-manage-row flex items-center gap-3 px-4 py-3"
          >
            <UAvatar
              :text="nameOf(a).charAt(0).toUpperCase()"
              size="sm"
    ...
    >
      <template #footer>
        ...
        <UButton
          ...
          icon="i-tabler-user-minus"
          color="error"
          :loading="!!confirmRemove && saving === confirmRemove.id"
          @click="doRemove"
        />
      </template>
    </UModal>
